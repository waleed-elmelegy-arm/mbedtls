...
static int verify_internal(const public_key_t *pub,
  ...
  size_t msg_len, const uint8_t *context, size_t context_len, uint8_t external_mu)
{
...
int tf_psa_crypto_mldsa87_verify(const uint8_t encoded_public_key[MLDSA87_PUBLIC_KEY_BYTES],
  const uint8_t encoded_signature[MLDSA87_SIGNATURE_BYTES], const uint8_t *msg,
  size_t msg_len, uint8_t external_mu)
{
  decode_public_key(&pub, encoded_public_key);
  return verify_internal(&pub, encoded_signature, msg, msg_len, NULL, 0, external_mu);
}
#else
#error "MLDSA is not defined"
